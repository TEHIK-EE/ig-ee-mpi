...
### Otsingu komponent
Paljudes tarkvarades on kasutusel patsiendi otsing ühe otsingu väljaga (Google stiilis). Et vältida liigsete andmete pärimist, kuna PÜT lisab kõik leitud patsiendid auditlogisse, soovitame ühtlustada ühe väljaga otsingut vastavalt allpoolt toodud kirjeldusele.

Allpool toodud pilt visualiseerib mõned otsingunäited: 1) otsing Eesti isikukoodiga; 2) välismaalase otsing, kus riigiks on määratud Portugal, perekonnanimi on Ronaldo, eesnimi on Cristiano ja sünnikuupäev on 05.02.1985; 3) tundmatu otsing.
...
- Keerulisem lahendus peaks arvestama [Patient Identifier Domain](https://build.fhir.org/ig/HL7EE/ig-ee-base/ValueSet-patient-identifier-domain.html) loendiga ja [Identifier Domain](https://build.fhir.org/ig/HL7EE/ig-ee-base/CodeSystem-identifier-domain.html) koodisüsteemi ülesehitusega. Iga loendi kuuluv mõiste omab tunnust `country` (riik), ning kõik kasutamiseks lubatud mõisted (`notSelectable`=false) omavad `idtype` (identifitseerimise viis). Iga riigiga on seotud erinev arv identifiseerimisviise. Näiteks, Soomes on lubatud identifitseerida ainult personaalse identifikaatori alusel, aga Saksamaal nii passinumbri kui ka ID-kaardi numbri põhjal. Arendajal on vabadus kas kasutada hierarhiat või kahte valiku (riik ja identifitseerimisviis) või oma lahendust, oluline et päring PÜTi tehakse koos identifitseerimissüsteemiga (nt Austria isikukoodiga `https://fhir.ee/sid/pid/aut/ni`). 
